...
class TestTemplate:
    ...
    def test_get_variable_selectors(self):
        """Test getting all variable selectors from a template."""
        template_str = "Hello {{#node1.name#}}, your age is {{#node2.age#}}."
        template = Template.from_answer_template(template_str)

        selectors = template.get_variable_selectors()
        assert len(selectors) == 2
        assert ["node1", "name"] in selectors
        assert ["node2", "age"] in selectors
