...
def test_parallel_branch_workflow_switch_scenarios():
    """
    Test parallel branch workflow with different switch values.

    This workflow tests parallel execution of code nodes based on switch value:
    - If switch == 1: Code 1 and Code 2 run in parallel, output {"1": "Code 1", "2": "Code 2", "3": null}
    - If switch != 1: Code 2 and Code 3 run in parallel, output {"1": null, "2": "Code 2", "3": "Code 3"}

    The current engine should fail with this test as it may not handle parallel execution correctly.
    """
    runner = TableTestRunner()

    test_cases = [
        WorkflowTestCase(
            fixture_path="parallel_branch_test",
            inputs={"switch": 1},
            expected_outputs={"1": "Code 1", "2": "Code 2", "3": None},
            description="Switch equals 1 - should run Code 1 and Code 2 in parallel",
        ),
        WorkflowTestCase(
            fixture_path="parallel_branch_test",
            inputs={"switch": 2},
            expected_outputs={"1": None, "2": "Code 2", "3": "Code 3"},
            description="Switch equals 2 - should run Code 2 and Code 3 in parallel",
        ),
        WorkflowTestCase(
            fixture_path="parallel_branch_test",
            inputs={"switch": 0},
            expected_outputs={"1": None, "2": "Code 2", "3": "Code 3"},
            description="Switch equals 0 - should run Code 2 and Code 3 in parallel",
        ),
        WorkflowTestCase(
            fixture_path="parallel_branch_test",
            inputs={"switch": -1},
            expected_outputs={"1": None, "2": "Code 2", "3": "Code 3"},
            description="Switch equals -1 - should run Code 2 and Code 3 in parallel",
        ),
        WorkflowTestCase(
            fixture_path="parallel_branch_test",
            inputs={"switch": 999},
            expected_outputs={"1": None, "2": "Code 2", "3": "Code 3"},
            description="Switch equals 999 - should run Code 2 and Code 3 in parallel",
        ),
    ]
    ...
    for result in results:
        assert result.success, f"Test case '{result.test_case.description}' failed: {result.error}"
        assert result.actual_outputs == result.test_case.expected_outputs, (
            f"Output mismatch for {result.test_case.description}. "
            f"Expected: {result.test_case.expected_outputs}, Got: {result.actual_outputs}"
        )
